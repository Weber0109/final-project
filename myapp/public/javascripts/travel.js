...
form.addEventListener("submit", (event) => {
    ...
    .then(data => {
        ...
        attractions.forEach(attraction => {
             // 檢查景點是否已經在 selectedList 中
            const isAlreadySelected = selectedList.some(selectedAttraction => selectedAttraction.Name === attraction.Name);
    ...
    function displaySelect() {
        ...
        selectedList.forEach((attraction, index) => {
            const row = document.createElement('tr');
            row.innerHTML = `
                <td>${attraction.Name}</td>
                <td><p>${attraction.Description}</p></td>
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', (e) => {
                ...
                selectedList.splice(index, 1); // 刪除景點
                displaySelect(); // 更新顯示
            });
        });
    }
